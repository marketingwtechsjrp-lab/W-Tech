...
3. Sitelinks, frases de destaque e snippets (não vão por planilha) — textos prontos na seção "Extensões" do relatório desta sessão; adicionar em Recursos → Sitelinks.
...
7. Tráfego de robô no GA4: ~125 "novos usuários" a cada 30 min batendo na home (EUA). Não afeta conversões, mas polui relatórios — vale filtrar (Admin → Fluxos de dados → lista de tráfego interno/IP, ou regra por país + sessão engajada).
