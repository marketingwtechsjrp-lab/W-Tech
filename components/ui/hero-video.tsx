...
const ContainerScroll = ({
  ...
  return (
    <ContainerScrollContext.Provider value={{ scrollYProgress }}>
      <section
        className={cn(
          "relative min-h-screen w-full pb-10 pt-8 ",
          className
        )}
        {...props}
        ref={scrollRef}
...
const ContainerInset = React.forwardRef<HTMLDivElement, ContainerInsetProps>(
  (
    ...
    const insetY = useTransform(scrollYProgress, progressRange, insetYRange, { clamp: true })
    const insetX = useTransform(scrollYProgress, progressRange, insetXRange, { clamp: true })
    const roundedness = useTransform(scrollYProgress, progressRange, roundednessRange, { clamp: true })
