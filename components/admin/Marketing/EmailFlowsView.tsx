...
const TRIGGER_COLORS: Record<TriggerType, string> = {
    ...
    Manual: 'bg-gray-100 text-gray-700 dark:bg-gray-700',
};
...
const StepCard = ({ step, index, onEdit, onDelete, isLast }: {
    ...
    return (
        <div className="flex flex-col items-center">
            <div className="group relative w-full max-w-md bg-[var(--admin-surface-2)] border border-[var(--admin-border)] rounded-2xl p-4 shadow-sm hover:shadow-md transition-all">
                <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-white flex-shrink-0 ${colors[step.type]}`}>
                        {icons[step.type]}
                    </div>
                    <div className="flex-1 min-w-0">
                        <p className="font-bold text-sm text-[var(--admin-text-primary)]">{labels[step.type]}</p>
                        <p className="text-xs text-[var(--admin-text-secondary)] truncate">{getDescription()}</p>
                    </div>
            ...
            {!isLast && (
                <div className="flex flex-col items-center my-1">
                    ...
                    <ArrowDown size={14} className="text-[var(--admin-text-tertiary)]" />
                </div>
            )}
        </div>
    );
...
const StepEditorModal = ({ step, onSave, onClose }: {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-[var(--admin-surface-1)] rounded-3xl shadow-2xl w-full max-w-lg border border-[var(--admin-border)]">
                <div className="flex items-center justify-between p-6 border-b border-[var(--admin-border)]">
                    <h3 className="font-black text-lg text-[var(--admin-text-primary)]">
                        ...
                        <>
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-2.5 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-surface-2)] text-sm focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white"
                                />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    className="w-full px-4 py-2.5 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-surface-2)] text-sm focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white resize-none"
                                />
                            </div>
                            <p className="text-xs text-gray-400">💡 Variáveis: <span className="font-mono bg-[var(--admin-surface-3)] px-1 rounded">{'{nome}'}</span>, <span className="font-mono bg-[var(--admin-surface-3)] px-1 rounded">{'{email}'}</span>, <span className="font-mono bg-[var(--admin-surface-3)] px-1 rounded">{'{produto}'}</span></p>
                        </>
                    )}
                    {step.type === 'Delay' && (
                ...
                <div className="flex gap-3 p-6 border-t border-[var(--admin-border)]">
                    <button onClick={onClose} className="flex-1 px-4 py-2.5 rounded-xl border border-[var(--admin-border)] text-sm font-bold text-[var(--admin-text-secondary)] hover:bg-gray-50 dark:hover:bg-[#333] transition-colors">Cancelar</button>
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                    <input
                        ...
                        className="text-2xl font-black bg-transparent border-none outline-none text-[var(--admin-text-primary)] w-full placeholder-gray-300 dark:placeholder-gray-600"
                    />
                </div>
                <div className="flex gap-2">
                    <button onClick={() => handleSave('Draft')} disabled={saving} className="px-4 py-2 rounded-xl border border-[var(--admin-border)] text-sm font-bold text-[var(--admin-text-secondary)] hover:bg-gray-50 dark:hover:bg-[#333] transition-colors">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-4">
                    <div className="bg-[var(--admin-surface-1)] rounded-2xl border border-[var(--admin-border)] p-5 space-y-4">
                        <h4 className="font-black text-sm text-[var(--admin-text-primary)] uppercase tracking-wider">Configurações</h4>
                        <div>
                            ...
                            <textarea
                                ...
                                className="w-full px-3 py-2 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-surface-2)] text-sm focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white resize-none"
                            />
                        </div>
                        <div>
                            ...
                            <select
                                ...
                                className="w-full px-3 py-2.5 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-surface-2)] text-sm focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white"
                            >
                    ...
                    {/* Add Step Buttons */}
                    <div className="bg-[var(--admin-surface-1)] rounded-2xl border border-[var(--admin-border)] p-5 space-y-3">
                        <h4 className="font-black text-sm text-[var(--admin-text-primary)] uppercase tracking-wider">Adicionar Step</h4>
                        <div className="grid grid-cols-2 gap-2">
                            {STEP_TYPES.map(st => (
                                <button key={st.type} onClick={() => addStep(st.type)}
                                    className="flex items-center gap-2 p-3 rounded-xl border border-[var(--admin-border)] hover:border-gray-300 dark:hover:border-gray-500 transition-all text-sm font-bold text-gray-700 hover:bg-gray-50 dark:hover:bg-[#333]">
                ...
                <div className="lg:col-span-2">
                    <div className="bg-[var(--admin-surface-1)] rounded-2xl border border-[var(--admin-border)] p-6 min-h-[500px]">
                        <h4 className="font-black text-sm text-[var(--admin-text-primary)] uppercase tracking-wider mb-6">Jornada do Fluxo</h4>
    ...
    return (
        <div className="bg-[var(--admin-surface-1)] border border-[var(--admin-border)] rounded-2xl p-5 hover:shadow-md transition-all">
            <div className="flex items-start justify-between gap-3 mb-4">
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${isActive ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : flow.status === 'Paused' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400' : 'bg-gray-100 text-gray-600 dark:bg-gray-800'}`}>
                            {isActive ? '● Ativo' : flow.status === 'Paused' ? '⏸ Pausado' : '○ Rascunho'}
                        </span>
                    </div>
                    <h3 className="font-black text-base text-[var(--admin-text-primary)]">{flow.name}</h3>
                    {flow.description && <p className="text-sm text-[var(--admin-text-secondary)] mt-0.5 line-clamp-1">{flow.description}</p>}
                </div>
            ...
            <div className="grid grid-cols-3 gap-2">
                <div className="bg-[var(--admin-surface-2)] rounded-xl p-2.5 text-center">
                    <p className="text-lg font-black text-[var(--admin-text-primary)]">{stats.enrolled}</p>
                    <p className="text-xs text-gray-500 mt-0.5">Enrolados</p>
                </div>
                <div className="bg-[var(--admin-surface-2)] rounded-xl p-2.5 text-center">
                    <p className="text-lg font-black text-[var(--admin-text-primary)]">{stats.active}</p>
...
const TemplateCard = ({ tpl, onUse }: { tpl: typeof FLOW_TEMPLATES[0]; onUse: (t: typeof FLOW_TEMPLATES[0]) => void | Promise<void>; key?: React.Key }) => (
    <button onClick={() => onUse(tpl)} className="group bg-[var(--admin-surface-1)] border border-[var(--admin-border)] rounded-2xl p-5 text-left hover:border-black dark:hover:border-white transition-all hover:shadow-lg">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-3 ${TRIGGER_COLORS[tpl.trigger_type]}`}>
            <GitBranch size={18} />
        </div>
        <h4 className="font-black text-sm text-[var(--admin-text-primary)] mb-1">{tpl.name}</h4>
        <p className="text-xs text-[var(--admin-text-secondary)] mb-3">{tpl.description}</p>
        <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-gray-400">{tpl.steps.length} steps</span>
            <span className="text-xs font-bold text-black group-hover:underline">Usar template →</span>
        </div>
    </button>
);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h3 className="text-xl font-black text-[var(--admin-text-primary)] flex items-center gap-2">
                        <GitBranch className="text-purple-500" size={22} />
                        Fluxos Automatizados
                    </h3>
                    <p className="text-sm text-[var(--admin-text-secondary)] mt-0.5">Crie jornadas automáticas de email com gatilhos, delays e condições.</p>
                </div>
                <div className="flex gap-2">
                    <button onClick={() => setView('templates')} className="px-4 py-2 rounded-xl border border-[var(--admin-border)] text-sm font-bold text-[var(--admin-text-secondary)] hover:bg-gray-50 dark:hover:bg-[#333] transition-colors flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                    ...
                    <div key={i} className="bg-[var(--admin-surface-1)] border border-[var(--admin-border)] rounded-2xl p-5 flex items-center justify-between">
                        <div>
                            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">{kpi.label}</p>
                            <p className="text-3xl font-black text-[var(--admin-text-primary)]">{kpi.value}</p>
                        </div>
            ...
            {view === 'templates' ? (
                <div>
                    <div className="flex items-center justify-between mb-4">
                        <h4 className="font-black text-base text-[var(--admin-text-primary)]">Templates Prontos</h4>
                ...
                <div className="flex flex-col items-center justify-center h-48 text-gray-400 gap-4">
                    ...
                    <div className="text-center">
                        <p className="font-bold text-[var(--admin-text-secondary)]">Nenhum fluxo criado ainda</p>
                        <p className="text-sm mt-1">Comece com um template ou crie do zero.</p>
                    </div>
                    <div className="flex gap-3">
                        <button onClick={() => setView('templates')} className="px-4 py-2 rounded-xl border border-[var(--admin-border)] text-sm font-bold hover:bg-gray-50 dark:hover:bg-[#333] transition-colors">Ver Templates</button>
