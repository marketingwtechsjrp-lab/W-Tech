        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-sm text-[var(--admin-text-secondary)]">Crie modelos para usar em agendamentos automáticos.</p>
                </div>
                {!isEditing && hasPerm('marketing_manage_templates') && (
                    <button 
                        onClick={() => { setIsEditing(true); setCurrentTemplate({ title: '', content: '' }); }}
                        className="bg-black text-white px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest flex items-center gap-2 hover:bg-gray-800 dark:bg-white dark:text-black transition-all shadow-lg active:scale-95"
                    >
            ...
            {isEditing && (
                <div className="bg-[var(--admin-surface-1)] rounded-2xl border-2 border-wtech-gold/20 dark:border-wtech-gold/10 p-8 mb-8 shadow-2xl animate-in zoom-in-95">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-xl font-black text-[var(--admin-text-primary)] uppercase tracking-tight flex items-center gap-2">
                            <Edit className="text-wtech-gold" /> {currentTemplate.id ? 'Editar Modelo' : 'Novo Modelo'}
                        </h3>
                        <button onClick={() => setIsEditing(false)} className="p-2 hover:bg-gray-100 dark:hover:bg-[#333] rounded-full transition-colors text-[var(--admin-text-secondary)]">
                            <X size={20} />
                        </button>
                    </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                        <div className="space-y-4">
                            <div>
                                ...
                                <input 
                                    className="w-full border border-[var(--admin-border)] rounded-xl p-3 text-sm font-bold outline-none focus:ring-2 focus:ring-black dark:focus:ring-white transition-all bg-[var(--admin-surface-2)]" 
                            ...
                            <div>
                                ...
                                <textarea 
                                    className="w-full border border-[var(--admin-border)] rounded-xl p-3 text-sm font-medium h-32 focus:ring-2 focus:ring-black dark:focus:ring-white outline-none bg-[var(--admin-surface-2)]" 
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="pt-2">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <input 
                                            type="number"
                                            className="w-24 border border-[var(--admin-border)] rounded-xl p-3 text-sm font-bold outline-none focus:ring-2 focus:ring-gray-200 dark:focus:ring-gray-600 bg-[var(--admin-surface-2)]" 
                                            min="0"
                                            max="60"
                                            value={currentTemplate.part_delay || 0}
                                            onChange={e => setCurrentTemplate({...currentTemplate, part_delay: parseInt(e.target.value) || 0})}
                                        />
                                        <span className="text-xs text-[var(--admin-text-secondary)]">Tempo de espera entre a Texto 1, Imagem e Texto 2.</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="mt-8 pt-6 border-t border-[var(--admin-border)] flex justify-end gap-3">
                         <button onClick={() => setIsEditing(false)} className="px-6 py-3 border border-[var(--admin-border)] rounded-xl text-sm font-bold text-[var(--admin-text-secondary)] hover:bg-gray-50 dark:hover:bg-[#333] transition-all">Cancelar</button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {templates.map(template => (
                    <div key={template.id} className="bg-[var(--admin-surface-1)] p-4 rounded-xl border border-[var(--admin-border)] shadow-sm hover:shadow-md transition-shadow group relative">
                        ...
                        <div className="text-sm text-[var(--admin-text-secondary)] bg-[var(--admin-surface-2)] p-3 rounded-lg border border-[var(--admin-border)] h-24 overflow-y-auto whitespace-pre-wrap custom-scrollbar">
                            {template.content}
                        </div>
                    </div>
                ))}
