...
export const NewOrderModal: React.FC<NewOrderModalProps> = ({ isOpen, onClose, onSave, onDelete, editingSale, user, initialItems = [] }) => {
    ...
    const handleSaveOrder = async () => {
        ...
        try {
            ...
            // Success animation/feedback could go here, but for now just close
            onClose();
            // Give it a tiny bit of time to start unmounting before refreshing data
            setTimeout(() => {
                onSave();
            }, 100);
        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div 
                    ...
                    className="fixed inset-0 z-[60] flex items-center justify-center p-0 md:p-4 bg-gray-900/60 backdrop-blur-sm cursor-pointer"
                >
                    <motion.div 
                        ...
                        className="bg-[#F8F9FC] dark:bg-[#121212] w-full max-w-7xl h-full md:h-[95vh] md:max-h-[1000px] md:rounded-[2rem] shadow-2xl overflow-hidden flex flex-col relative cursor-default"
                    >
                         {/* Header */}
