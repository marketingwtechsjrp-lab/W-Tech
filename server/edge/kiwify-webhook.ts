 ...
 * `supabase/functions/kiwify-webhook`). URL: /api/kiwify-webhook
 *
 * Lógica preservada 1:1; só mudou o runtime:
 *   - Deno.env.get('X')  → process.env.X
 *   - serve()/Response   → handler Express (res.status().json())
 *   - cliente Supabase criado sob demanda (lazy) para o server não crashar no
 *     boot quando as envs ainda não existem.
 */
...
// No Supabase gerenciado a env se chama SUPABASE_URL; no restante do projeto
// (functions da Vercel) usa-se VITE_SUPABASE_URL. Aceita as duas.
...
export default async function kiwifyWebhookHandler(req: Request, res: Response) {
  try {
    ...
    if (!supabase) {
      console.error('[Kiwify Webhook] Missing env vars: SUPABASE_URL/VITE_SUPABASE_URL ou SUPABASE_SERVICE_ROLE_KEY');
    ...
    try {
      ...
      console.error("[Kiwify Webhook] DB Config error, using env fallback:", dbErr);
    }
    ...
    if (!phone) {
      ...
      return res.status(200).json({ message: "Acknowledged, but no action taken (no phone)" });
    }
    ...
    return res.status(200).json({ success, message: "Webhook processed" });
  } catch (err: any) {
